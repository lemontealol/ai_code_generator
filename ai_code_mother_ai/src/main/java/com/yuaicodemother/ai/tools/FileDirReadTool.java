...
public class FileDirReadTool extends BaseTool{
    ...
    public String readDir(
        ...
        try {
            ...
            // 使用 Hutool 递归获取所有文件
            List<File> allFiles = FileUtil.loopFiles(targetDir, file -> !shouldIgnore(file.toPath(),projectRoot));
            // 按路径深度和名称排序显示
            allFiles.stream()
                    .sorted((f1, f2) -> {
...
//                        String indent = "  ".repeat(depth);
                        String depth = getRelativePath(file, projectRoot);
                        structure.append(depth).append("\n");
                    });
    ...
    private String getRelativePath(File targetDir, Path projectPath) {
        Path targetDirPath = targetDir.toPath();
        return projectPath.relativize(targetDirPath).toString();
    }
    /**
     * 判断是否应该忽略该文件或目录
     */
    private boolean shouldIgnore(Path targetDir, Path rootPath) {
        // 获取相对路径
        Path relativePath = rootPath.relativize(targetDir);
        // 检查路径中的每一部分
